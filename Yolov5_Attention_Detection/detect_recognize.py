...
sys.path.append("..") 
from LPR_Attention_Recognition.model.LPRAtt import get_label, vec2text
...
@torch.no_grad()
def detect(opt):
    source, weights, view_img, save_txt, imgsz = opt.source, opt.d_weights, opt.view_img, opt.save_txt, opt.img_size
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or opt.save_crop or view_img:  # Add bbox to image
                        ...
                        if opt.save_crop:
                            ...
                            label = get_label(opt.r_weights, crop_img)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--d_weights', nargs='+', type=str, default='./weights/best.pt', help='model.pt path(s)')
    parser.add_argument('--r_weights', nargs='+', type=str, default='../LPR_Attention_Recognition/weight/Final_LPRAtt_model.pth', help='model.pt path(s)')
    ...
    if opt.update:  # update all models (to fix SourceChangeWarning)
        for opt.d_weights in ['yolov5s.pt', 'yolov5m.pt', 'yolov5l.pt', 'yolov5x.pt']:
            detect(opt=opt)
            strip_optimizer(opt.d_weights)
    else:
        detect(opt=opt)
