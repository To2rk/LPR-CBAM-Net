...
class MobileNetV3(nn.Module):
    ...
    def __init__(
        ...
        self.classifier1 = nn.Sequential(
                ...
                nn.Linear(last_channel, provNum),
            )
        self.classifier2 = nn.Sequential(
            nn.Linear(lastconv_output_channels, last_channel),
            nn.Hardswish(inplace=True),
            nn.Dropout(p=0.2, inplace=True),
            nn.Linear(last_channel, alphaNum),
        )
        self.classifier3 = nn.Sequential(
            nn.Linear(lastconv_output_channels, last_channel),
            nn.Hardswish(inplace=True),
            nn.Dropout(p=0.2, inplace=True),
            nn.Linear(last_channel, adNum),
        )
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        y1 = torch.argmax(x1, dim=1).view(-1, 1)
        y2 = torch.argmax(x2, dim=1).view(-1, 1)
        y3 = torch.argmax(x3, dim=1).view(-1, 1)
        y4 = torch.argmax(x4, dim=1).view(-1, 1)
        y5 = torch.argmax(x5, dim=1).view(-1, 1)
        y6 = torch.argmax(x6, dim=1).view(-1, 1)
        y7 = torch.argmax(x7, dim=1).view(-1, 1)

        output = torch.cat((y1, y2, y3, y4, y5, y6, y7), 1)

        return [x1, x2, x3, x4, x5, x6, x7], output
...
def vec2text(ouput):
    label_list =[]
    ouput = ouput.cpu().tolist()
    label_list.append(provinces[ouput[0][0]])
    label_list.append(alphabets[ouput[0][1]])

    for i in range(5):

        label_list.append(ads[ouput[0][i+2]])
